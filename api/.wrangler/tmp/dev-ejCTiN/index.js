...
// .wrangler/tmp/bundle-YcJL7e/checked-fetch.js
...
var hasD1 = /* @__PURE__ */ __name((c) => c.env?.DB, "hasD1");
app.get("/", (c) => {
  return c.json({
    status: "ok",
...
app.get("/api/tasks/:id", async (c) => {
  ...
  if (hasD1(c)) {
    try {
      const task2 = await c.env.DB.prepare("SELECT * FROM tasks WHERE id = ?").bind(id).first();
      if (task2) return c.json(task2);
...
app.post("/api/tasks", async (c) => {
  ...
  if (hasD1(c)) {
    try {
      ...
      const newTask2 = await c.env.DB.prepare("SELECT * FROM tasks WHERE id = ?").bind(result.meta.last_row_id).first();
      return c.json(newTask2, 201);
    } catch (e) {
...
app.get("/api/projects/:id", async (c) => {
  ...
  if (hasD1(c)) {
    try {
      const project2 = await c.env.DB.prepare("SELECT * FROM projects WHERE id = ?").bind(id).first();
      if (project2) {
        const { results: tasks } = await c.env.DB.prepare("SELECT * FROM tasks WHERE project_id = ?").bind(id).all();
        return c.json({ ...project2, tasks });
      }
...
app.post("/api/projects", async (c) => {
  ...
  if (hasD1(c)) {
    try {
      ...
      const newProject = await c.env.DB.prepare("SELECT * FROM projects WHERE id = ?").bind(result.meta.last_row_id).first();
...
app.get("/api/milestones", async (c) => {
  ...
  if (hasD1(c)) {
    try {
      ...
      if (projectId) {
        const { results: results2 } = await c.env.DB.prepare(query + " WHERE project_id = ? ORDER BY id").bind(projectId).all();
...
// ../../../../Users/arkan/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../../../Users/arkan/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-YcJL7e/middleware-insertion-facade.js
...
// ../../../../Users/arkan/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-YcJL7e/middleware-loader.entry.ts
